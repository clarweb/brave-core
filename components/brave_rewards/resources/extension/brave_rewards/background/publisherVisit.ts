...
const handleMediaDurationMetadataResponse = (tabId: number, mediaType: string, data: MediaDurationMetadataResponse) => {
  ...
  chrome.braveRewards.getPublisherInfo(publisherKey, (result: number, info?: RewardsExtension.Publisher) => {
...
const handleSavePublisherVisitResponse = (tabId: number, mediaType: string, data: SavePublisherVisitResponse) => {
  ...
  chrome.braveRewards.getPublisherInfo(
    data.publisherKey,
    (getResult: RewardsExtension.Result, info?: RewardsExtension.Publisher) => {
      console.debug(`getPublisherInfo: result=${getResult}`)
      ...
      if (getResult === 9) {
        chrome.braveRewards.savePublisherInfo(
          ...
          (result: RewardsExtension.Result) => {
            if (result !== 0) {
              console.error(`Failed to save publisher info for ${data.publisherKey}, result is ${result}`)
              return
            }
          })
        return
