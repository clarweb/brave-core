...
LedgerDatabase::LedgerDatabase(const std::string& path) :
    db_path_(path),
    initialized_(false) {
...
void LedgerDatabase::RunTransaction(
...
DBCommandResponse::Status LedgerDatabase::Initialize(
...
DBCommandResponse::Status LedgerDatabase::Execute(DBCommand* command) {
  if (!initialized_) {
    return DBCommandResponse::Status::INITIALIZATION_ERROR;
  }
...
DBCommandResponse::Status LedgerDatabase::Run(DBCommand* command) {
  if (!initialized_) {
    return DBCommandResponse::Status::INITIALIZATION_ERROR;
  }
...
DBCommandResponse::Status LedgerDatabase::Read(
    DBCommand* command,
    DBCommandResponse* command_response) {
  if (!initialized_) {
    return DBCommandResponse::Status::INITIALIZATION_ERROR;
...
DBCommandResponse::Status LedgerDatabase::Migrate(
    const int32_t version,
    const int32_t compatible_version) {
  if (!initialized_) {
    return DBCommandResponse::Status::INITIALIZATION_ERROR;
